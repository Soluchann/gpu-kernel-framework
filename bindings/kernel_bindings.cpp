...
// Use the new HAL and portable kernel header
...
py::array_t<uint16_t> eltwise_add_fp16_wrapper(py::array_t<uint16_t> A_arr, py::array_t<uint16_t> B_arr) {
    ...
    // Use the portable gpuHalf type
    std::vector<gpuHalf> h_A(N), h_B(N), h_Out(N);

    for (int i = 0; i < N; i++) {
        h_A[i] = *reinterpret_cast<gpuHalf*>(&A[i]);
        h_B[i] = *reinterpret_cast<gpuHalf*>(&B[i]);
    }
...
py::array_t<uint16_t> matmul_fp16_wrapper(
    ...
    const void *A = bufA.ptr;
    const void *B = bufB.ptr;

    py::array_t<uint16_t> result = py::array_t<uint16_t>({M * N});
    void *C = result.mutable_data();

    // The launch function now correctly handles the void* pointers
    launch_matmul_fp16(A, B, C, M, N, K);
